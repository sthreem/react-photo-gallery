...
export const selectPhotos = (state: RootState) =>
  helpers.sortByCreatedAt([...state.photos.photos]);
export const selectFavorites = (state: RootState) =>
  helpers.sortByCreatedAt([...state.photos.favorites]);
...
export const selectPhotoSize = (state: RootState) =>
  helpers.convertPhotoSize(state.photos.photos);
export const selectIsPhotoInFavorites = (photoId: string) => (state: RootState) =>
  state.photos.favorites.some((photo) => photo.id === photoId);
export const selectInfoToDisplay = (state: RootState) =>
  helpers.extractInfoToDisplay(state.photos.selectedPhoto);
