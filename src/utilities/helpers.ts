...
const formatInfo = (
  key: string,
  value: string | number | boolean | PhotoMetrics | SharedUser[],
): string => {
  switch (key) {
    case 'createdAt':
    case 'updatedAt':
...
export default {
  ...
  convertPhotoSize: (photos: Photo[]) => {
    return photos.reduce<{ [key: string]: string }>((result, photo) => {
      result[photo.id.toString()] = `${(photo.sizeInBytes / (1024 * 1024)).toFixed(
        1,
      )} MB`;
      return result;
    }, {});
  ...
  extractInfoToDisplay: (photo: Photo | null) => {
    ...
    return Object.entries(photo)
      .filter(([key]) =>
        Object.keys(constants.INFO_TO_DISPLAY).includes(key as keyof Photo),
      )
      .map(([key, value]) => ({
        key: constants.INFO_TO_DISPLAY[key as keyof Photo],
        value: formatInfo(key, value as string),
      }));
  },
};
